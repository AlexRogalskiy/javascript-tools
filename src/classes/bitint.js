...
jsar.utils.bitInt = function(spec) {
	var bitset = [];
	//var that = {};
	var that = Object.create(jsar.utils.bitInt.prototype);
	...
	var init = function() {	
		if(!jsar.toolset.isNull(spec)) {
			...
			if(Object.prototype.hasOwnProperty.call(spec, 'bitset')/*spec.hasOwnProperty('bitset')*/ && jsar.toolset.isArray(spec.bitset)) {
				bitset = spec.bitset.slice(0);
			} else if(Object.prototype.hasOwnProperty.call(spec, 'length')/*spec.hasOwnProperty('length')*/ && jsar.toolset.isIntNumber(spec.length) && spec.length > 0) {
				for(var i=0; i<spec.length; i++) {
					bitset[i] = 0;
				}
			} else if(Object.prototype.hasOwnProperty.call(spec, 'number')/*spec.hasOwnProperty('number')*/ && jsar.toolset.isIntNumber(spec.number) && spec.number >= 0) {
				var n = 0;
				while(spec.number) {
				...
				throw jsar.exception.argumentException({name: 'ArgumentException', message: 'incorrect initialization value: {\'bitset\': [array of {0, 1}]} or {\'number\': [integer number] >= 0} or {\'length\': [integer number] > 0}'});
				//throw {
				//	name: 'ValueError',
	...
	var fullSubstractor = function(borrow) {
		return function(b1, b2) {
			var diff;
			if(borrow) {
				diff = !(b1 ^ b2);
	...
	that.bitsetLtEq = function(y) {
		...
		var len = (this.getLength() < y.getLength()) ? this.getLength() : y.getLength();
	...
	that.bitsetAdd = function(y) {
		...
		var len = (this.getLength() < y.getLength()) ? this.getLength() : y.getLength();
		var _fullAdder = fullAdder(false);
		for(var i=0; i<len; i++) {
	...
	that.bitsetMultiply = function(y) {
		...
		var len = (this.getLength() < y.getLength()) ? this.getLength() : y.getLength();
		var temp = jsar.utils.bitInt(this.getLength());
		var tempX = this.clone(), tempY = y.clone();
	...
	that.bitsetDivide = function(y) {
		...
		var q = jsar.utils.bitInt(this.getLength()), r = jsar.utils.bitInt(this.getLength());
		if(this.isEquals(y)) {
			q[0] = 1;
			return {'quotient': q, 'residue': r};
		}
		r = jsar.utils.bitInt({'bitset': bitset});
		if(this.bitsetLt(y)) {
			return;
		}
		var len = (this.getLength() < y.getLength()) ? this.getLength() : y.getLength();
	...
	that.shiftLeft = function(n) {
		if(!jsar.toolset.isIntNumber(i) || i < 0 || i > (bitset.length - 1)) { throw {
																				name: 'ValueError',
	...
	that.invert = function() {
		bitset = bitset.map(function(item) { return item === 1 ? 0 : 1; });
		return this;
	};
	that.clone = function() {
		return jsar.utils.bitInt({'bitset': bitset});
	};
	that.nativeCopy = function() {
		var res = [];//bitset.slice(0);
