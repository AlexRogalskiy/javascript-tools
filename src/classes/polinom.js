...
jsar.utils.polinom = function(spec) {
	...
	var init = function() {
		if(!jsar.toolset.isNull(spec)) {
			...
			if(Object.prototype.hasOwnProperty.call(spec, 'polinom')/*spec.hasOwnProperty('polinom')*/ && jsar.toolset.isArray(spec.polinom)) {
				polinom1 = spec.polinom.slice(0);
			} else if(Object.prototype.hasOwnProperty.call(spec, 'degree')/*spec.hasOwnProperty('degree')*/ && jsar.toolset.isIntNumber(spec.degree) && spec.degree > 0) {
					polinom1 = jsar.toolset.vector(spec.degree + 1, 0);
			} else {
				throw jsar.exception.argumentException({name: 'ArgumentException', message: 'incorrect initialization value: {\'polinom\': [array of coefficients]} or {\'degree\': [integer number] > 0}'});
				//throw {
				//	name: 'ValueError',
	...
	that.polinomMult = function(polinom2) {
		if(!isPolinom(polinom2)) { throw {
										name: 'ValueError',
										message: 'not polinom instance: < ' + y + ' >'
								};
		}
		var tempPolinom2 = polinom2.clone();
		var n = ((polinom1.length > tempPolinom2.length) ? polinom1.length : tempPolinom2.length), tmp = jsar.toolset.vector(2*n, 0);
	...
	that.getValue = function(x) {
		if(!Math.toolset.isNumber(x)) { throw {
											name: 'ValueError',
	...
	that.getValues = function(low, high, step) {
		if(!jsar.toolset.isNumber(low) || !jsar.toolset.isNumber(high) || !jsar.toolset.isIntNumber(step) || low >= high || step <= 0) { throw {
																																			name: 'ValueError',
																																			message: 'incorrect input value: lower point < ' + low + '>, upper point < ' + high + '>, step < ' + step + ' >'
																																		};
		}
		var h = (high - low) / (step - 1);
		var arrayX = jsar.toolset.vector(step, 0), arrayY = jsar.toolset.vector(step, 0);
	...
	that.getCoeffAt = function(i) {
		if(!jsar.toolset.isIntNumber(i) || i < 0 || i > (polinom1.length - 1)) { throw {
																					name: 'ValueError',
	...
	that.equals = function(polinom2) {
		...
		var tempPolinom1 = this.clone(), tempPolinom2 = polinom2.clone();
