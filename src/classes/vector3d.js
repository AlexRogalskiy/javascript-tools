		...
		var vector3d = function(spec) {
			...
			var init = function() {
				if(!globals.toolset.isNull(spec)) {
					...
					if(Object.prototype.hasOwnProperty.call(spec, 'x') && Object.prototype.hasOwnProperty.call(spec, 'z') && Object.prototype.hasOwnProperty.call(spec, 'y')/*spec.hasOwnProperty('x') && spec.hasOwnProperty('y') && spec.hasOwnProperty('z')*/ && globals.toolset.isNumber(spec.x) && globals.toolset.isNumber(spec.y) && globals.toolset.isNumber(spec.z)) {
						vx = spec.x;
						vy = spec.y;
						vz = spec.z;
					} else {
			...
			that.scaleXYZ = function(scaleX, scaleY, scaleZ) {
				if(!globals.toolset.isNumber(scaleX) || !globals.toolset.isNumber(scaleY) || !globals.toolset.isNumber(scaleZ)) { throw {
																														name: 'ValueError',
																														message: 'incorrect scale values: scaleX < ' + scaleX + ' >, scaleY < ' + scaleY + ' >, scaleZ < ' + scaleZ + ' >'
																													};
				}
				vx *= scaleX;
				vy *= scaleY;
		...
		/*	that.rotate = function(angle) {
			...
			*/
			that.rotate = function(angle) {
				...
				var theta = (vx > 0) ? Math.atan2(vy, vx) : (vx < 0) ? (Math.PI + Math.atan2(vy, vx)) : (vx === 0 && vy >= 0) ? Math.PI / 2 : 3 * Math.PI / 2;
				//
				var vxx = vx, vyy = vy, cosVal = Math.cos(theta), sinVal = Math.sin(theta);
			...
			that.rotateX = function(angle) {
				...
				var vyy = vy, vzz = vz, cosVal = Math.cos(angle), sinVal = Math.sin(angle);
			...
			that.rotateY = function(angle) {
				...
				var vxx = vx, vzz = vz, cosVal = Math.cos(angle), sinVal = Math.sin(angle);
				vx = vxx * cosVal + vzz * sinVal;
				vz = -vxx * sinVal + vzz * cosVal;
				return this;
			};
			that.rotateZ = function(angle) {
				...
				var vxx = vx, vyy = vy, cosVal = Math.cos(angle), sinVal = Math.sin(angle);
				vx = vxx * cosVal - vyy * sinVal;
				vy = vxx * sinVal + vyy * cosVal;
				return this;
			};
			that.shift = function(deltaX, deltaY, deltaZ) {
				if(!globals.toolset.isNumber(deltaX) || !globals.toolset.isNumber(deltaY) || !globals.toolset.isNumber(deltaZ)) { throw {
																														name: 'ValueError',
																														message: 'incorrect shift values: deltaX < ' + deltaX + ' >, deltaY < ' + deltaY + ' >, deltaZ < ' + deltaZ + ' >'
																													};
				}
				vx += deltaX;
				vy += deltaY;
			...
			that.toSphereCoordinates = function() {
				var vr = this.length(), vpsi = Math.asin(vz / vr), vphi = vy / Math.sqrt(vx * vx + vy * vy);
			...
			that.setCoordY = function(y) {
				if(!globals.toolset.isNumber(y)) { throw {
													name: 'ValueError',
