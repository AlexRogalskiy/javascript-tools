...
//----------------------------------------------------------------------------------------------
		const Logger = (function() {
			...
			that.setLogLevel = function(logLevel) {
				...
				if(!globals.toolset.isIntNumber(logLevel)) { throw {
															name: 'TypeError',
				...
				if(-1 == Object.values(globals.utils.logger.LEVEL).indexOf(state)) {
					throw globals.exception.argumentException('OutOfBoundsError', 'incorrect input argument: logLevel < ' + logLevel + ' > is invalid');
				}
				_logLevel = logLevel;
			};
			...
			that.info = function() {
				...
				var slice = Array.prototype.slice, args = slice.apply(arguments, [0]);
			...
			that.assert = function(expr) {
				if(!jsar.toolset.isBoolean(expr)) {
					throw jsar.exception.typeException({name: 'TypeError', message: 'incorrect boolean value: < ' + expr + ' >'});//new TypeError('');
				}
