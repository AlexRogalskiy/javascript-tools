				...
				that.remove = function() {
					if(_next != null) {
						if(_next != _head && _current != null) {
							...
							if(_current == null) 
								...
								_next.getNext().setPrevious(_next.getNext());
							_current = _head;
							delete _next;
							_next = null;
						}
					...
					if(_previous != null) {
						if(_previous != _head && _current != null) {
							...
							if(_current == null) 
								_previous.getNext().setPrevious(_current);
							else 
								_previous.getNext().setPrevious(_previous.getNext());
							_current = _head;
							delete _previous;
							_previous = null;
						}
			...
			(function() {
				...
				that.equals = function(node) {
					...
					return ((_data == node.getData() || (_data != null && _data.equals(node.getData()))) &&
							(_previous == node.getPrevious()() || (_previous != null && _previous.equals(node.getPrevious()))) &&
...
//----------------------------------------------------------------------------------------------		
		(function() {
			...
			that.findBeginning = function() {
				...
				if(fast == null || fast.getNext() == null) {//fast == _start && fast.getNext() == _start
					return null;
				}
			...
			that.isRadar = function(node) {
				...
				var isRadar_ = function(head, len) {
					if(head == null || len == 0) { //(head.getNext() == _start)
