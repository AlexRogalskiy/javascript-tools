...
//----------------------------------------------------------------------------------------------
	const isList = (value) => (value instanceof globals.collections.list.List);
//----------------------------------------------------------------------------------------------
	/* @public
	* @module collections
	...
	*
	* @example
	* var myList = globals.collections.list([1, 4, 5, 5, 6, 7]);
	*/
	(function() {
			...
			(function() {
				...
				that.compareTo = function(node) {
					...
					if(!(node instanceof globals.collections.list.node.ListNode)) {
						throw {
							name: 'TypeError',
							message: 'incorrect input parameter: not ListNode instance  < ' + node + ' >'
						};
					}
		...
		(function() {
			...
			that.findMToLastElement = function(m) {
				if(!globals.toolset.isIntNumber(m)) { throw {
														name: 'ValueError',
														message: 'incorrect input value: not integer number < ' + m + ' >'
													};
				}
				if(this.isEmpty()) return null;
				if(m <= 0) return null;
			...
			that.entries = function() {
				if(this.isEmpty()) return null;
				var result = globals.toolset.vector(), current = _start;
				do {
			...
			that.clone = function() {
				return new globals.collections.list.List(this.entries(), compare);
			};
			...
			globals.collections.list.List = List;
		}());
//----------------------------------------------------------------------------------------------
	}());
