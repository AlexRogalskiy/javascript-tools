...
//----------------------------------------------------------------------------------------------
    ...
    (function () {
      ...
      function BinSet(nodes, maxVal, _bins, cmp) {
        ...
        _maxSize =
          maxVal == null ? DEFAULT_SIZE : globals.toolset.isIntNumber(maxVal) && maxVal > 0 ? maxVal : null;
        if (_maxSize == null)
          throw { name: 'ValueError', message: 'incorrect max size value: < ' + _maxSize + ' >' };

        _bins = bins == null ? DEFAULT_BINS : globals.toolset.isIntNumber(bins) && bins > 0 ? bins : null;
        if (_bins == null)
          throw { name: 'ValueError', message: 'incorrect {number of _bins} value: < ' + _bins + ' >' };
