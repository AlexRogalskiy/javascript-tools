(function (globals) {
  ...
  //----------------------------------------------------------------------------------------------
  const isTree = value => value instanceof globals.collections.tree.Tree;
  //----------------------------------------------------------------------------------------------
  /* @public
   * @module collections
   ...
   *
   * @example
   * var myBinaryTree = new globals.collections.tree.Tree([1, 4, 5, 5, 6, 7]);
   */
  (function () {
    ...
    (function () {
      ...
      (function () {
        ...
        that.isYoungest = function () {
          return _youngest;
        };
        that.setYoungest = function (youngest) {
          _youngest = youngest;
        };
        that.compareTo = function (node) {
          ...
          if (!(node instanceof globals.collections.tree.node.TreeNode)) {
            throw {
              name: 'TypeError',
              message: 'incorrect input parameter: not TreeNode instance  < ' + node + ' >',
            };
          }
        ...
        that.equals = function (node) {
          ...
          if (!(node instanceof globals.collections.tree.node.TreeNode)) return false;
          ...
          return (
            ...
            (_youngest == node.isYoungest() || (_youngest != null && _youngest.equals(node.isYoungest()))) &&
        ...
        that.hashCode = function () {
          ...
          sfVal = _youngest == null ? 0 : _youngest.hashCode();
          hashValue = 31 * hashValue + sfVal;
        ...
        that.toString = function () {
          return '[data: {' + _data.toString() + '}, youngest: {' + _youngest.toString() + '}]';
        };

        function TreeNode(data, son, brother, youngest) {
          _data = Object.clone(data);
          _son = son;
          _brother = brother;
          _youngest = youngest;
        }
        TreeNode.prototype = that;

        globals.collections.tree.node.TreeNode = TreeNode;
      })();
    })();
    //----------------------------------------------------------------------------------------------
    (function () {
    ...
    (function () {
      ...
      that.insertRoot = function (item) {
        ...
        var current = _root;
        var nodeLeft = new globals.collections.tree.node.TreeNode(item, null, null);
      ...
      that.outputAsc = function () {
        var result = globals.toolset.vector(),
          current = _root,
          stack = new globals.collections.stack.Stack();
      ...
      that.depth = function (node) {
        ...
        if (!(node instanceof globals.collections.tree.node.TreeNode)) {
          throw {
            name: 'TypeError',
            message: 'incorrect input argument: not TreeNode instance < ' + node + ' >',
          };
        }
        return depth_(node);
      };
      ...
      that.findSum = function (node, sum) {
        ...
        if (!globals.toolset.isNumber(sum)) {
          throw {
            name: 'ValueError',
      ...
      that.findLowestCommonAncestor2 = function (node, child1, child2) {
        ...
        if (!(child1 instanceof globals.collections.tree.node.TreeNode)) {
          throw {
            name: 'TypeError',
            message: 'incorrect input argument: not TreeNode instance {child1} < ' + child1 + ' >',
          };
        }
        if (!(child2 instanceof globals.collections.tree.node.TreeNode)) {
          throw {
            name: 'TypeError',
            message: 'incorrect input argument: not TreeNode instance {child2} < ' + child2 + ' >',
          };
        }
      ...
      that.isBST = function (node) {
        var isBST_ = function (node, min, max) {
          ...
          if (
            !checkBST_(node.getLeft(), min, node.getData()) ||
            !checkBST_(node.getRight(), node.getData(), max)
          ) {
            return false;
          }
          return true;
      ...
      that.nodesOnLevel = function (level) {
        ...
        if (!globals.toolset.isIntNumber(level)) {
          throw {
            name: 'TypeError',
        ...
        if (level < 0) {
          throw globals.exception.argumentException(
            'OutOfBoundsError',
            'incorrect input argument: level < ' + level + ' > is negative'
          );
        }
      ...
      globals.collections.tree.Tree = Tree;
    })();
    //----------------------------------------------------------------------------------------------
  })();
