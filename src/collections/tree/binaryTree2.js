...
//----------------------------------------------------------------------------------------------
		var btSMF = function(level, node) {
			if(!globals.toolset.isIntNumber(level) || !globals.toolset.isIntNumber(node) || level < 0 || node < 0) { throw {
																												name: 'ValueError',
...
//----------------------------------------------------------------------------------------------			
		(function() {
			...
			that.has = function(value) {
				...
				this.root();
				var current = this.getNode(), order;
				while(current !== null) {
