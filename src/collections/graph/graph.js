...
//----------------------------------------------------------------------------------------------
		(function() {
			...
			that.add = function(item) {
				var node = new globals.collections.graph.node.GraphNode(item.data);
				_queue.add(node);
				//_src[_src.length] = node;
			};
			that.setBaconNumbers = function() {
				var _queue_temp = new globals.collections.list.LinkedList();
				for(var current in _queue.entries()) {
					if(current != null) {
						for(var n in current.getSet().entries()) {
							if(-1 === n.getData()) {
