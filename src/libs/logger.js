...
const getOutputStyle = type =>
  `color: ${  DEFAULT_COLORS_PRESET[type] ? DEFAULT_COLORS_PRESET[type] : DEFAULT_COLORS_PRESET['black']}`;
...
const getLocalTime = (format, offset = 0, utc = false) => {
  ...
  const currentTime = currentDate.getTime() + (currentDate.getTimezoneOffset() * 60000);
  const localCurrentTime = new Date(currentTime + (offset * 3600000));
