...
if (!Array.prototype.splice)
  ...
  if (ind>len){ 
   ...
   let removeArray = this.slice(ind,ind+cnt);
   let endArray = this.slice(ind+cnt);
   len = this.length = ind;
...
Array.prototype.subtract = function (a2, compareFunction) {
  if (!compareFunction) compareFunction=null;
   let a1=this.removeDuplicates(compareFunction);
  if (!a2) return a1;
...
Array.prototype.removeDuplicates = function (compareFunction) {
  if (!compareFunction) compareFunction=null;
   let a1=this.concat().sort(compareFunction);
   if (compareFunction){ 
      for (var i=0;i<a1.length;i++){ 
         var src=a1[i];
...
Array.prototype.unsortedSubtract = function (a2, compareFunction) {
  if (!compareFunction) compareFunction=null;
   let a1=this.unsortedRemoveDuplicates(compareFunction);
   if (!a2) return a1;
...
Array.prototype.unsortedExclusion = function (a2, compareFunction) {
  if (!compareFunction) compareFunction=null;
   ...
   if (!a2) return a1;
  return a1
    .unsortedSubtract(a2, compareFunction)
    .concat(a2.unsortedSubtract(a1, compareFunction))
