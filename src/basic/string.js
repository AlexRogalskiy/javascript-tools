(function (globals) {
  ...
  (function () {
     ...
     * @returns {string} The given input as split by chars/letters
     */
     ...
     * @param {string} str The string to transform
     * @param {string} tmpl Template that gets interpolated
     * @returns {string} The given input split by lines
     */
    String.method('wrapChars', function (tmpl) {
      tmpl = isString(tmpl) ? tmpl : '<span>$&</span>';
    ...
    String.method('getWholeChar', function (i) {
      ...
      const prev = this.charCodeAt(i - 1);
    ...
    //----------------------------------------------------------------------------------------------
    String.method('camelize', function () {
      const strList = this.split('-/=*_+|(){}[]');
      if (strList.length === 1) {
        return strList[0];
      }
      let strCamelized =
        this.indexOf('-') == 0 ? strList[0].charAt(0).toUpperCase() + strList[0].substring(1) : strList[0];
    ...
    String.method('knuthMorrisPratt', function (begin, pattern) {
      ...
      const pf = globals.toolset.vector(pattern.length, 0);
    ...
    String.method('isDate', function () {
      return /(0?[1-9]|1[0-2])\/([0-2]?[1-9]|[1-3][01])\/(\d\d)(\s)+([01]?[1-9]|[12][0-4]):([0-5]\d):([0-6]\d)?/.test(
        this
      );
    });
    //----------------------------------------------------------------------------------------------
    ...
    String.method('getWordAt', function (i) {
      const temp = this.getWords();
      if (!globals.toolset.isIntNumber(i) || i < 0 || i > temp.length - 1) {
        throw {
          name: 'ValueError',
    ...
    String.method('getCharAt', function (i) {
      ...
      if (!globals.toolset.isIntNumber(i) || i < 0) {
        throw {
          name: 'ValueError',
          message: `incorrect input value: char position < ${i} >`,
        };
    ...
    String.method('insertAt', function (str, n) {
      if (!globals.toolset.isString(str) || !globals.toolset.isIntNumber(n)) {
        throw {
          name: 'ValueError',
    ...
    String.method('nthSubstr', function (str, n, repeats) {
      ...
      const adv = repeats ? 1 : str.length;
    ...
    String.method('repeatNum', function (str, repeats) {
      if (!globals.toolset.isString(str)) {
        throw {
          name: 'ValueError',
    ...
    String.method('startsWith', function (txt) {
      if (!globals.toolset.isString(txt)) {
        throw {
          name: 'ValueError',
    ...
    String.method('isRotation', function (str) {
      const isSubstring = function (big, small) {
      ...
      if (len == str.length && len > 0) {
        const s1s1 = this.toString() + this.toString();
    ...
    String.method('lpad', function (padString, length) {
      if (!globals.toolset.isIntNumber(length) || length < 0) {
        throw {
          name: 'ValueError',
    ...
    String.method('replaceAll', function (searchValue, replacer) {
      if (!globals.toolset.isString(searchValue) || !globals.toolset.isString(replacer)) {
        throw {
          name: 'ValueError',
    ...
    //});
    //----------------------------------------------------------------------------------------------
    String.method('occurrences', function (searchValue, matchCase) {
      if (!globals.toolset.isString(searchValue)) {
        throw {
          name: 'ValueError',
    ...
    String.method('repeat', function (times) {
      if (!globals.toolset.isIntNumber(times) || times < 0) {
        throw {
          name: 'ValueError',
    ...
    //----------------------------------------------------------------------------------------------
    String.method('countOccurrentces', function (regex) {
      if (!globals.toolset.isRegExp(regex)) {
        throw {
          name: 'ValueError',
    ...
    String.method('replaceAt', function (index, character) {
      if (!globals.toolset.isIntNumber(index) || index < 0) {
        throw {
          name: 'ValueError',
    ...
    String.method('removeChars', function (chars) {
      if (!globals.toolset.isString(chars)) {
        throw {
          name: 'ValueError',
    ...
    String.method('cycle', function (k) {
      if (!globals.toolset.isIntNumber(k) || k < 0) {
        throw {
          name: 'ValueError',
...
//document.body.innerHTML.occurrences("div"); //google home page has 114
//document.body.innerHTML.occurrences("/div"); //google home page has 57
//"England engages its engineers".occurrentces("eng",true); //2
...
//var regex = /x/g;
//document.writeln('_x_x'.occurrences('x'));
//document.writeln('_x_x'.countOccurrentces(regex));
